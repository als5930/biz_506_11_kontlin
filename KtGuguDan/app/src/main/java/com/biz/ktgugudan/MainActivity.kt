...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // EditText에 키보드러 입력을 하면 Change Event가 발생한다
        ...
        // 미보드로 EditText box 에 문자를 입력했을때
        // 문자열이 입력된 사항을 지켜보고 있다가
        // 문자열의 처리를 담당하는 역할을 수행한다
        // 람바 코드로 익명 클래스를 선언 한다
        txt.addTextChangedListener(object :TextWatcher{
            ...
            override fun onTextChanged(s: CharSequence?, start: Int, before: Int, count: Int) {
                ...
                val text = txt.text
                // 직접 view의 text속성에 문자열을 부착하여 app 화면에 보여주기
                // app device입장에서는 상당히 많은 비용이 소요되는 코드
                //view?.text = text
                subject.map { txt.text.toString()  == ""}
                    .flatMap ({ BehaviorSubject.range(1,9) })
            // 최초에 app이 실행되거나 EditText에 문자열이 모두 제거 되면 {1,2,~9}형태의 배열자료를 만든어라
                    // java strem(연속된 데이터) 형태의 자료를 생서하는 method
                    // flaMap에 의해 생성된 데이터는 Observable형태의 데이터이다
                    // Observable관련된 method등을 chain하여 데이터를 처리할수 있다
                    {_,row-> "0 x $row = 0\n "}
                    .scan{x,y -> x + y}
                 ...
                 * 만약 text change가 발생했늘때 화면에 그리기 위한 text문자열을 만드는 코드가 많아진다면
                 * (for, while등을 이용하여 코드를 작성해야 할 것이다)
                 * 그 코드에 의해 문자열이 모두 생성되어소 화면에 나타날때 까지
                 * mainActivity의 다른 코드나 이벤트의 실행에 영향을 미치게 된다
                 * 
                 * subject를  사용하게되면 change event가 발생한 즉시 subject에게 데이터를 넘겨버리고
                 * mainActivty를 작동사키는 main thread는 역할을 끝내버린다
